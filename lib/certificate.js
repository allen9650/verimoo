...
export async function buildCertificateSVG({ project, participant, verifyUrl }) {
  ...
  if (project?.templateImage?.data) {
    ...
    // Optimize large raster background templates so the SVG payload stays well below Vercel's 4.5MB limit
...
export async function svgToPngBuffer(svgString, width = 1000, height = 700) {
  ...
  try {
    const sharpInstance = sharp || (await import("sharp")).default;
    ...
    try {
      const sharpInstance = sharp || (await import("sharp")).default;
      return await sharpInstance(Buffer.from(svgString)).png().toBuffer();
    } catch (fallbackErr) {
      console.warn("Sharp rasterization unavailable in this environment:", fallbackErr);
      return null;
    }
  }
}
...
export async function generateCertificatePdf(svgString, width = 1000, height = 700, title = "Certificate of Verification") {
  ...
  if (!pngBuffer) {
    throw new Error("Failed to render high-resolution PNG for PDF compilation.");
  }
