...
export async function GET(req, { params }) {
  ...
  if (isRateLimited(`verify:${getClientKey(req)}`, 60)) {
    return NextResponse.json({ valid: false, message: "Too many requests. Please slow down and try again shortly." }, { status: 429 });
  }

  await connectDB();
  const participant = await Participant.findOne({ serialNumber: serial });
  if (!participant) {
    return NextResponse.json({ valid: false, message: "No certificate found with this serial number." }, { status: 404 });
  }
